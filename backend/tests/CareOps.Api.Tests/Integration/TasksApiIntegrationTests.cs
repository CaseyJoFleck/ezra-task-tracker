...
public class TasksApiIntegrationTests : IClassFixture<CareOpsApiFactory>
{
    ...
    [Fact]
    public async Task List_tasks_search_finds_mri_in_title()
    {
        await _factory.ResetDatabaseAsync();
        var client = _factory.CreateClient();

        var res = await client.GetAsync("/api/tasks?search=MRI");
        res.StatusCode.Should().Be(HttpStatusCode.OK);
        var list = await res.Content.ReadFromJsonAsync<List<TaskItemResponse>>(ApiTestJson.Options);
        list.Should().NotBeNull();
        list!.Should().Contain(t => t.Title.Contains("MRI", StringComparison.OrdinalIgnoreCase));
    }
